...
// TODO: Passing data to variables (normalize)
  ...
  const getInfoAllowed = async (roleOfRequestingUser, idOfRequestingUser, user)=>{
    ...
    // OWNER USER PERMISSION: owner user can modifiy restricted info.
    else if (roleOfRequestingUser ==='user' && idOfRequestingUser === user.idUsers) {
      ...
      return res.status(400).json({error: "Vous n'avez pas les permis necessaires"})
    }
...
exports.deleteUser = (req, res) => {
  ...
  if (roleOfRequestingUser === 'admin' || idOfRequestingUser === userToDelte) {
    ...
    return res.status(400).json({error: "Vous devez être administrateur pour effacer la compte d'une autre personne"});
  }
